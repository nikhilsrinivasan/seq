...
if(geoPosition.init()){
	...
	console.log('Modernizr thinks we can get location but it was not possible.');
}

// for simulating locations
// geoPositionSimulator.init(); 

function success_callback(p){
	// p is location object
	for(var i in p.coords){
		if(p.coords[i] !== null){
			// add updated coordinates to page
			if($('#p_id_'+i).length)
				$('#p_id_'+i).val(p.coords[i]);
			else
				$('#main-search-form').append('<input type="hidden" name="p_'+i+'" value="'+p.coords[i]+'" id="p_id_'+i+'">');
			
			if(Modernizr.localstorage) // store coordinates for next use, in case geoLocation does not return quickly enough or is unavailable
				localStorage.setItem('p_'+i,p.coords[i]);
		}
	}
}
